...
{       
    if (enablelighting) {       
        ...
        {
          if (lightposn[i][3]==0) //directional light
          {
		    // Light 0, directional
          ...
          if (lightposn[i][3]==1) //point light 
          {
	    	// Light 1, point 
		    vec3 position = lightposn[i].xyz / lightposn[i].w ; 
		    vec3 direction1 = normalize (position - mypos) ; // no attenuation 
		    vec3 half1 = normalize (direction1 + eyedirn) ;  
		    vec4 col1 = ComputeLight(direction1, lightcolor[i], normal, half1, diffuse, specular, shininess) ;
		    finalcolor = finalcolor + col1; 
          }
        }
